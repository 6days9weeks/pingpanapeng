...
class Afk(commands.Cog):
    ...
    @commands.command(name="toggleafk")
    @checks.admin_or_permissions(administrator=True)
    async def _ignore(self, ctx):
        """
        Toggle afk messages on the whole server.
    ...
    async def away_settings(self, ctx):
        """View your current afk settings"""
        author = ctx.author
        msg = ""
        data = {
            "MESSAGE": "Away",
        ...
        if ctx.channel.permissions_for(ctx.me).embed_links:
            ...
            em.set_author(name=f"{author.display_name}'s afk settings", icon_url=author.avatar_url)
            await ctx.send(embed=em)
        else:
            await ctx.send(f"{author.display_name} afk settings\n" + msg)