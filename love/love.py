...
class LoveCal(commands.Cog):
    ...
    async def love(
        ...
        who: Optional[FuzzyMember],
        whom: Optional[FuzzyMember] = None,
    ) -> None:
        """
        ...
        def ship_url(love_percent, who, whom):
            url2 = (
                "https://api.martinethebot.com/v1"
                + "/imagesgen/ship?percent="
                + str(love_percent)
                + "&first_user="
                + urllib.parse.urlencode(url=who.avatar_url)
                + "&second_user="
                + urllib.parse.urlencode(url=whom.avatar_url)
            )
            return url2
