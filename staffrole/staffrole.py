...
class StaffRoles(commands.Cog):
    ...
    async def add_staff(self, ctx):
        ...
        try:
            ...
            try:
                await ctx.guild.get_member(ctx.thread.recipient.id).add_roles(ctx.guild.get_role(int(roles['staff'])), reason="Role Added By Echidna")
                await ctx.send(embed=discord.Embed(description="Added <@&"+roles['staff']+"> to "+ctx.thread.recipient.mention, color=0xD0B8D6))
            except discord.Forbidden:
                await ctx.send(embed=discord.Embed(description="Failed to add <@&"+roles['staff']+"> to "+ctx.thread.recipient.mention, color=0xD0B8D6))
        except KeyError:
    ...
    async def add_helper(self, ctx):
        ...
        try:
            ...
            try:
                ...
                await ctx.guild.get_member(ctx.thread.recipient.id).add_roles(ctx.guild.get_role(int(roles['helper'])), reason="Role Added By Echidna")
                await ctx.send(embed=discord.Embed(description="Added <@&"+roles['helper']+"> to "+ctx.thread.recipient.mention, color=0xD0B8D6))
            except discord.Forbidden:
                await ctx.send(embed=discord.Embed(description="Failed to add <@&"+roles['helped']+"> to "+ctx.thread.recipient.mention, color=0xD0B8D6))
        except KeyError:
            await ctx.send(embed=discord.Embed(description="helper role not found", color=0xD0B8D6))
