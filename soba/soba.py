...
class StatusNezuko(commands.Cog):
    ...
    async def start_status(self):
        ...
        await asyncio.sleep(10)
        await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=f"Soba is mine - ឵឵❥sasha"))
        await asyncio.sleep(10)
        await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Nezuko Hideout | DM For Help | {server.member_count} Members"))
